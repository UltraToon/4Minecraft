...
while true; do
  ...
  case "$ACTION" in
  "Info")
    osascript -e 'display dialog "ATLauncher is a custom Minecraft launcher for modpacks and instances.\nRecommended if you have used CurseForge or Prism Launcher before." buttons {"Back"} with title "Info"'
    ;;
  "Troubleshooting")
    osascript -e 'display dialog "Common fixes:\n• Re-run this script if Java errors appear\n• Go to Finder->Documents->MCSEHS and delete all Java folders\n• Try a lower/higher Java version if Minecraft crashes" buttons {"Back"} with title "Troubleshooting"'
    ;;
  "Set Up") break ;;
  esac
done
